...
GraphiteComposer.prototype = {
  ...
  toggleD3: function(is_d3) {
    this.state.D3 = is_d3;
    if (is_d3) {
    // D3 mode is on.
      this.url.addParam("format", "json");
      d3.select("#g3-viewer")
        .style("width", "100%")
	.style("height", "100%")
	...
	.html("")
      d3.select('#image-viewer')
        .style('visibility', 'visible')
      this.url.removeParam("format", "json");
    }
  },

  d3GraphTypeSelected: function(name) {
    console.log("Composer selecting graph type with name: " + name);
    this.state.D3_type = name;
    this.updateImage();
  },
  ...
  updateImage: function () {
    ...
    if (this.state.D3) {
      // Empty the graph div.
      document.getElementById('g3-viewer').innerHTML = "";
      switch(this.state.D3_type) {
       case "linegraph": 
         // Make a D3 line graph.  
         var g = new G3.LineGraph('g3-viewer');
         var xhr = new XMLHttpRequest();
         xhr.open("GET", this.url.getURL());
         xhr.onreadystatechange = function() {
           if (xhr.readyState == 4) {
	     console.log(JSON.parse(xhr.responseText));
             g.setData(JSON.parse(xhr.responseText));
           }
         }
         xhr.send();
         break;
       case "histogram":
	     ...
	     //console.log(JSON.parse(xhr.responseText));
             //g.setData(JSON.parse(xhr.responseText));
             var g = new G3.Histogram('g3-viewer', JSON.parse(xhr.responseText));
           }
         }
         xhr.send();
         break;
 	 default:
	 // The default case is just the line graph.
 	 // But it's redundantly copypastad here for clarity up
         // above.   
         // Make a D3 line graph.  
         var g = new G3.LineGraph('g3-viewer');
         var xhr = new XMLHttpRequest();
      ...
      // Do the traditional image.
      var img = this.window.getImage();
      console.log(this.window.getImage);
      if (img) {
        var now = new Date();
