...
function createComposerWindow(myComposer) {
  ...
  function d3GraphTypeSelected(combo, record, index) {
    console.log("" + combo + " " + record + " " + index);
    switch (index) {
      case 0:
        Composer.d3GraphTypeSelected("linegraph");
        console.log("Line Graph selected");
        break;
      case 1:
        Composer.d3GraphTypeSelected("histogram");
  ...
  var g3GraphTypeCombo = new Ext.form.ComboBox({ 
	 xtype: 'combo',
	 id: 'g3type_combo',
	 fieldLabel: 'd3type',
	 emptyText: 'D3 Graph Type',
 	 triggerAction: 'all',
	 width: 120,
         editable: false,
	 /*disabled: true,*/
	 store: [ 'Line Graph', 'Histogram' ],
   	 listeners: {
	   select: d3GraphTypeSelected
	 }
  });
  ...
  var bottomToolbar = [
    ...
    { text: "Image / D3", id: 'g3toggle_button', enableToggle: true, toggleHandler: toggleG3 },
    g3GraphTypeCombo
  ];
  ...
  var win = new Ext.Window({
    ...
    buttonAlign: 'left',
    items: { html: "<img id='image-viewer' src='/render'/><div id='g3-viewer'></div>", region: "center" },
    listeners: {
...
function toggleG3(button, pressed) {
  Composer.toggleD3(pressed);
  Composer.updateImage();
}
